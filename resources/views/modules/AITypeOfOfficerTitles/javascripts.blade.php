...
$(document).ready(function() {
    func_buttonClick = function(p_type, p_id){
        if( p_type == 'add'){
            $('#actionModal .modal-title').text("Thêm mới danh mục chức vụ cán bộ");
            $('#form-action')[0].reset();
            $('#form-action #id_input_action_type').val('add');
        } 
        else if ( p_type == 'edit') {
            $('#actionModal .modal-title').text("Chỉnh sửa danh mục chức vụ cán bộ");
            ...
            $('#deleteModal .modal-header .modal-title').html('Xóa danh mục chức vụ cán bộ');
            $('#deleteModal .question-to-confirm').html('Bạn có chắc muốn xóa danh mục chức vụ cán bộ không? ');
        }
    };
    /* ------------------------------------------ Handle ------------------------------------------*/
    $("#form-action").on("submit", function( event ) {
        ...
        $url = '{!! route('AITypeOfOfficerTitles.post.ajax') !!}';
