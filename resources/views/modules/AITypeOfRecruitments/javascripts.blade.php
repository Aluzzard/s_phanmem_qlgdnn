...
$(document).ready(function() {
    func_buttonClick = function(p_type, p_id){
        if( p_type == 'add'){
            $('#actionModal .modal-title').text("Thêm mới danh mục đối tượng tuyển sinh / ưu tiên");
            $('#form-action')[0].reset();
            $('#form-action #id_input_action_type').val('add');
        } 
        else if ( p_type == 'edit') {
            $('#actionModal .modal-title').text("Chỉnh sửa danh mục đối tượng tuyển sinh / ưu tiên");
            ...
            $('#deleteModal .modal-header .modal-title').html('Xóa danh mục đối tượng tuyển sinh / ưu tiên');
            $('#deleteModal .question-to-confirm').html('Bạn có chắc muốn xóa danh mục đối tượng tuyển sinh / ưu tiên không? ');
        }
    };
    /* ------------------------------------------ Handle ------------------------------------------*/
    $("#form-action").on("submit", function( event ) {
        ...
        $url = '{!! route('AITypeOfRecruitments.post.ajax') !!}';
