...
$(document).ready(function() {
    func_loaddt = function() {
        ...
        url = '{!! route('AIOfficerWorkProcess.post.ajax') !!}';
...
/* ----------------------------------------------------------------------------------------*/
    func_buttonClick = function(action_type, id_btn){
        if ( action_type == 'viewList') {
            ...
            $('#form_action')[0].reset();
            $('h4#form_category_title').html('Quản lý quá trình công tác');
            ...
            $("#deleteModal h5.modal-title").html('Xóa quá trình công tác');
    ...
    func_viewWorkProcess = function(p_id){
        $.ajax({
            ...
            success: function(data) {
                console.log(data)
                $("#actionModal h4#form_title").html('Chỉnh sửa: <span style="color: red;">'+data.officer.information+'</span>');
                $("#actionModal input[name=action_type]").val('edit');
                $("#actionModal input[name=id]").val(data.officer.id);
                $("#actionModal input[name=information]").val(data.officer.information);
