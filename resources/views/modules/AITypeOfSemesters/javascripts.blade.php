...
$(document).ready(function() {
    func_buttonClick = function(p_type, p_id){
        if( p_type == 'add'){
            $('#actionModal .modal-title').text("Thêm mới danh mục học kỳ");
            $('#form-action')[0].reset();
            $('#form-action #id_input_action_type').val('add');
        } 
        else if ( p_type == 'edit') {
            $('#actionModal .modal-title').text("Chỉnh sửa danh mục học kỳ");
            ...
            $('#deleteModal .modal-header .modal-title').html('Xóa danh mục học kỳ');
            $('#deleteModal .question-to-confirm').html('Bạn có chắc muốn xóa danh mục học kỳ không? ');
        }
    };
    /* ------------------------------------------ Handle ------------------------------------------*/
    $("#form-action").on("submit", function( event ) {
        ...
        $url = '{!! route('AITypeOfSemesters.post.ajax') !!}';
