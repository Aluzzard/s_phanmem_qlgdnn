...
namespace App\Http\Controllers\Modules\AITypeOfOfficerTitles;
...
//Models
use App\Models\Modules\ModuleTypeOfOfficerTitles;
//Requests
use App\Http\Requests\Module\ModuleTypeOfOfficerTitlesRequest;
//Controllers
use App\Http\Controllers\MainStructure\Admins\LogsController;
use App\Http\Controllers\MainStructure\Users\GlobalFunction;

class AITypeOfOfficerTitlesController extends Controller {
    ...
    public function index() {
        if(GlobalFunction::checkRoleModuleByUser('view') == false || GlobalFunction::checkRoleFunctionByUser('view',200) == false){
            ...
            return view('modules.AITypeOfOfficerTitles.index', $data);
        }
    }
    ...
    public function dataTable(){
        ...
        $listofpeoples = ModuleTypeOfOfficerTitles::whereIdSite($current_id_site)->get();
        $html = '';
        if($listofpeoples){
    ...
    public function view($request){
        try {
            $people = ModuleTypeOfOfficerTitles::find( $request->input('id') );
    ...
    public function add($request){
        try {
            $validator = Validator::make($request->all(), ModuleTypeOfOfficerTitlesRequest::rules($request->input('id')), ModuleTypeOfOfficerTitlesRequest::messages());
            if($validator->fails()){
                return response()->json(['error' => true, 'validate' => false, 'message' => $validator->messages()->all()]);
            } else {
                $input = $request->all();
                $input['id_site'] = Auth::guard('user')->user()->id_site;
                ModuleTypeOfOfficerTitles::create($input);
                $html = $this->dataTable($request->input());
                LogsController::write("AITypeOfOfficerTitles", "Thêm", "Chức vụ cán bộ: ".$input["name"]);
                return response()->json(['error'=> false, 'message' => 'Thêm chức vụ cán bộ thành công', 'table_html' => $html]);
            }
        } catch (Exception $e) {
    ...
    public function edit($request){
        try {
            ...
            if($validator->fails()){
                ...
                $people->update($input);
                $html = $this->dataTable($request->input());
                LogsController::write("AITypeOfOfficerTitles", "Sửa", "Chức vụ cán bộ: ".$input["name"]);
                return response()->json(['error'=> false, 'message' => 'Sửa chức vụ cán bộ thành công', 'table_html' => $html]);
            }
        } catch (Exception $e) {
    ...
    public function delete($request){
        try {
            ...
            $people = ModuleTypeOfOfficerTitles::find( $people_id );
            $people->delete();
            LogsController::write("AITypeOfOfficerTitles", "Xoá", "Chức vụ cán bộ: ".$people->name);
            
            $html = $this->dataTable($request->input());
            return response()->json(['error'=> false, 'message' => 'Xóa chức vụ cán bộ thành công', 'table_html' => $html]);
        } catch (Exception $e) {
