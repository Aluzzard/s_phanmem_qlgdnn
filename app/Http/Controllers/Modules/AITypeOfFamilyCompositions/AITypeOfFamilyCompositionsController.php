...
namespace App\Http\Controllers\Modules\AITypeOfFamilyCompositions;
...
//Requests
use App\Http\Requests\Module\ModuleTypeOfFamilyCompositionsRequest;
//Controllers
use App\Http\Controllers\MainStructure\Admins\LogsController;
use App\Http\Controllers\MainStructure\Users\GlobalFunction;

class AITypeOfFamilyCompositionsController extends Controller {
    ...
    public function index() {
        if(GlobalFunction::checkRoleModuleByUser('view') == false || GlobalFunction::checkRoleFunctionByUser('view',200) == false){
            ...
            return view('modules.AITypeOfFamilyCompositions.index', $data);
        }
    }
    ...
    public function dataTable(){
        ...
        $listofpeoples = ModuleTypeOfFamilyCompositions::whereIdSite($current_id_site)->get();
        $html = '';
        if($listofpeoples){
    ...
    public function view($request){
        try {
            $people = ModuleTypeOfFamilyCompositions::find( $request->input('id') );
    ...
    public function add($request){
        try {
            $validator = Validator::make($request->all(), ModuleTypeOfFamilyCompositionsRequest::rules($request->input('id')), ModuleTypeOfFamilyCompositionsRequest::messages());
            if($validator->fails()){
                return response()->json(['error' => true, 'validate' => false, 'message' => $validator->messages()->all()]);
            } else {
                $input = $request->all();
                $input['id_site'] = Auth::guard('user')->user()->id_site;
                ModuleTypeOfFamilyCompositions::create($input);
                $html = $this->dataTable($request->input());
                LogsController::write("AITypeOfFamilyCompositions", "Thêm", "Thành phần gia đình: ".$input["name"]);
                return response()->json(['error'=> false, 'message' => 'Thêm thành phần gia đình thành công', 'table_html' => $html]);
            }
        } catch (Exception $e) {
            return response()->json(['error'=> true, 'message' => 'Thêm thành phần gia đình thất bại']);
        }
    }
    public function edit($request){
        try {
            ...
            if($validator->fails()){
                ...
                $people->update($input);
                $html = $this->dataTable($request->input());
                LogsController::write("AITypeOfFamilyCompositions", "Sửa", "Thành phần gia đình: ".$input["name"]);
                return response()->json(['error'=> false, 'message' => 'Sửa thành phần gia đình thành công', 'table_html' => $html]);
            }
        } catch (Exception $e) {
             return response()->json(['error'=> true, 'message' => 'Sửa thành phần gia đình thất bại']);
        }
    }
    public function delete($request){
        try {
            $people_id = $request->input('id');
            $people = ModuleTypeOfFamilyCompositions::find( $people_id );
            $people->delete();
            LogsController::write("AITypeOfFamilyCompositions", "Xoá", "Thành phần gia đình: ".$people->name);
            
            $html = $this->dataTable($request->input());
            return response()->json(['error'=> false, 'message' => 'Xóa thành phần gia đình thành công', 'table_html' => $html]);
        } catch (Exception $e) {
             return response()->json(['error'=> true, 'message' => 'Xóa thành phần gia đình thất bại']);
        }
    }
}
