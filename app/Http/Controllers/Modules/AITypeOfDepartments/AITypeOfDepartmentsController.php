...
namespace App\Http\Controllers\Modules\AITypeOfDepartments;
...
//Models
use App\Models\Modules\ModuleTypeOfDepartments;
//Requests
use App\Http\Requests\Module\ModuleTypeOfDepartmentsRequest;
//Controllers
use App\Http\Controllers\MainStructure\Admins\LogsController;
use App\Http\Controllers\MainStructure\Users\GlobalFunction;

class AITypeOfDepartmentsController extends Controller {
    ...
    public function index() {
        if(GlobalFunction::checkRoleModuleByUser('view') == false || GlobalFunction::checkRoleFunctionByUser('view',200) == false){
            ...
            return view('modules.AITypeOfDepartments.index', $data);
        }
    }
    ...
    public function dataTable(){
        ...
        $listofpeoples = ModuleTypeOfDepartments::whereIdSite($current_id_site)->get();
        $html = '';
        if($listofpeoples){
            foreach ($listofpeoples as $people){
                $html .= '<tr>';
                $html .= '  <td>'.$people->code.'</td>';
                $html .= '  <td>'.$people->name.'</td>';
                $html .= '  <td>'.$people->founding_date.'</td>';
    ...
    public function view($request){
        try {
            $people = ModuleTypeOfDepartments::find( $request->input('id') );
    ...
    public function add($request){
        try {
            $validator = Validator::make($request->all(), ModuleTypeOfDepartmentsRequest::rules($request->input('id')), ModuleTypeOfDepartmentsRequest::messages());
            if($validator->fails()){
                return response()->json(['error' => true, 'validate' => false, 'message' => $validator->messages()->all()]);
            } else {
                $input = $request->all();
                $input['id_site'] = Auth::guard('user')->user()->id_site;
                ModuleTypeOfDepartments::create($input);
                $html = $this->dataTable($request->input());
                LogsController::write("AITypeOfDepartments", "Thêm", "Khoa/phòng: ".$input["name"]);
                return response()->json(['error'=> false, 'message' => 'Thêm khoa/phòng thành công', 'table_html' => $html]);
            }
        } catch (Exception $e) {
            return response()->json(['error'=> true, 'message' => 'Thêm khoa/phòng thất bại']);
        }
    }
    public function edit($request){
        try {
            ...
            if($validator->fails()){
                ...
                $people->update($input);
                $html = $this->dataTable($request->input());
                LogsController::write("AITypeOfDepartments", "Sửa", "Khoa/phòng: ".$input["name"]);
                return response()->json(['error'=> false, 'message' => 'Sửa khoa/phòng thành công', 'table_html' => $html]);
            }
        } catch (Exception $e) {
             return response()->json(['error'=> true, 'message' => 'Sửa khoa/phòng thất bại']);
        }
    }
    public function delete($request){
        try {
            $people_id = $request->input('id');
            $people = ModuleTypeOfDepartments::find( $people_id );
            $people->delete();
            LogsController::write("AITypeOfDepartments", "Xoá", "Khoa/phòng: ".$people->name);
            
            $html = $this->dataTable($request->input());
            return response()->json(['error'=> false, 'message' => 'Xóa khoa/phòng thành công', 'table_html' => $html]);
        } catch (Exception $e) {
             return response()->json(['error'=> true, 'message' => 'Xóa khoa/phòng thất bại']);
        }
    }
}
