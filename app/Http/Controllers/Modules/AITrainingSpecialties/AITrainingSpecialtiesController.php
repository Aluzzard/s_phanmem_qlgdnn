...
namespace App\Http\Controllers\Modules\AITrainingSpecialties;
...
class AITrainingSpecialtiesController extends Controller {
    ...
    public function index() {
        if(GlobalFunction::checkRoleModuleByUser('view') == false || GlobalFunction::checkRoleFunctionByUser('view',200) == false){
            ...
            return view('modules.AITrainingSpecialties.index', $data);
        }
    }
    ...
    public function dataTable(){
        ...
        $items = ModuleTrainingSpecialties::whereIdSite($current_id_site)
                                            ->orderby('type_1')
                                            ->orderby('type_2')
                                            ->get();
        $html = '';
        if($items){
            foreach ($items as $item){
                ...
                if($item->type_1 == 1){
                    $html .= '  <td>Đào tạo trình độ dưới 03 tháng</td>';
                } else {
                    $html .= '  <td>Đào tạo trình độ sơ cấp</td>';
                }
                if($item->type_2 == 1){
                    $html .= '  <td>Phi nông nghiệp</td>';
                } else {
                    $html .= '  <td>Nông nghiệp</td>';
                }
            
                $html .= '  <td>'.$item->duration_by_month.'</td>';
                $html .= '  <td>'.$item->duration_by_day.'</td>';
                $html .= '  <td>';
    ...
    public function view($request){
        try {
            $item = ModuleTrainingSpecialties::find( $request->input('id') );
    ...
    public function add($request){
        try {
            $validator = Validator::make($request->all(), ModuleTrainingSpecialtiesRequest::rules($request->input('id')), ModuleTrainingSpecialtiesRequest::messages());
            if($validator->fails()){
                return response()->json(['error' => true, 'validate' => false, 'message' => $validator->messages()->all()]);
            } else {
                $input = $request->all();
                $input['id_site'] = Auth::guard('user')->user()->id_site;
                ModuleTrainingSpecialties::create($input);
                $html = $this->dataTable($request->input());
                LogsController::write("AITrainingSpecialties", "Thêm", "Khóa học: ".$input["name"]);
    ...
    public function edit($request){
        try {
            ...
            if($validator->fails()){
                ...
                $item->update($input);
                $html = $this->dataTable($request->input());
                LogsController::write("AITrainingSpecialties", "Sửa", "Khóa học: ".$input["name"]);
    ...
    public function delete($request){
        try {
            ...
            $item = ModuleTrainingSpecialties::find( $item_id );
            $item->delete();
            LogsController::write("AITrainingSpecialties", "Xoá", "Khóa học: ".$item->name);
